...
"VPC끼리, 또는 서비스에 **인터넷을 안 거치고** 어떻게 연결하지?" — 상황에 따라 세 가지 답이 있다.
...
Peering·TGW는 **네트워크(VPC)를 연결**한다. PrivateLink는 네트워크를 잇지 않고
**서비스 하나만 콕 집어** 프라이빗하게 노출한다 — 양쪽 네트워크가 서로 안 보여 더 격리적.
</KeyPoint>
